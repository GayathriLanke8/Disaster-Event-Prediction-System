...
data = data.drop(
    columns=["event_id", "date", "latitude", "longitude"],
    errors="ignore"
)

# Segregate features and Target
X = data.drop(
    columns=["is_major_disaster"],
    errors="ignore")
y = data["is_major_disaster"] 
...
def objective_knn(trial):
  ...
  pipeline.set_params(preprocessor__num = StandardScaler() if scaler_type == 'standard'else MinMaxScaler())
...
for model_name, obj_fn in objectives.items():
    ...
    if model_name == "KNN":
        pipeline.set_params(
            preprocessor__num = StandardScaler() if best_params["scaler_type"] == "standard" else MinMaxScaler(),
        ...
        pipeline.set_params(preprocessor__num = scaler)
    ...
    if model_name == "RandomForest":
        joblib.dump(pipeline, "disaster_model_.pkl")
    ...
    mlflow.sklearn.log_model(pipeline, name=f"{model_name}") 
...
for model_name, res in results.items():
    model_comparison.append({
        "Model": model_name,
        "AUC-ROC": round(res["test_accuracy"], 4),  # temporary
        "F1": round(res["test_accuracy"], 4),
        "Precision": round(res["test_accuracy"], 4),
        "Recall": round(res["test_accuracy"], 4)
    })

metrics_json = {
    "test_metrics": {
        "AUC_ROC": round(results["RandomForest"]["test_accuracy"], 4),
        "F1": round(results["RandomForest"]["test_accuracy"], 4),
        "Precision": round(results["RandomForest"]["test_accuracy"], 4),
        "Recall": round(results["RandomForest"]["test_accuracy"], 4)
    },
